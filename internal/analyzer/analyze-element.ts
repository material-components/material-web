...
export function analyzeFields(
  ...
  for (const field of classDeclaration.fields) {
    // skip certain fields
    if (FIELDS_TO_IGNORE.has(field.name)) {
      continue;
    }
 ...
 * generation.
 *
 * @param classDeclaration The LitElement class declaration from which to analyze and format the method data.
...
export function analyzeMethods(
  ...
  for (const method of classDeclaration.methods) {
    // Skip methods we decided to skip
 ...
 *
 * @param classDeclaration The LitElement class declaration from which to analyze and format the event data.
