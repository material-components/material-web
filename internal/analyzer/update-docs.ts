...
async function updateApiDocs() {
  ...
  const documentationFileNames = Object.keys(
    docsToElementMapping
  ) as DocFileName[];

  const filesWritten: Promise<void>[] = [];
 ...
 * @param analyzer The instance of the package analyzer from which to pull module information.
 ...
 * @param packagePath The path of the package root.
 * @param analyzer An instance of the package analyzer from which to pull module information.
 *
...
function generateFieldMarkdownTable(element: MdModuleInfo): MarkdownTable {
  ...
  /**
   * Formats and appends a property to the propertiesTable.
   *
   * @param property The property to format append to the table's rows.
   */
  const appendPropertyToTableRows = (property: MdPropertyInfo) => {
  ...
  while (currentClass) {
    for (const property of currentClass.reactiveProperties) {
      appendPropertyToTableRows(property);
    }
